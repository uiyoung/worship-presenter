...
export async function getHymnImagesById(req, res, next) {
  ...
  const imageDirectory = `./public/resources/hymn/images/${no}`;

  try {
    const readIndexPromise = fs.readFile('public/resources/hymn/index.json', 'utf8');
...
export async function updateLyrics(req, res, next) {
  try {
    ...
    const hymn = await fs.readFile(`public/resources/hymn/lyrics/${no}.json`, 'utf8');
    const hymnObj = JSON.parse(hymn);
    const newHymn = { ...hymnObj, verses: newVerses };
    await fs.writeFile(`public/resources/hymn/lyrics/${no}.json`, JSON.stringify(newHymn));
